...
public class UserInfoAction extends BaseAction {
    ...
    public String AddUserInfo() {
        ...
        try {
            Area areaObj = areaDAO.GetAreaByAreaId(userInfo.getAreaObj().getAreaId());
            userInfo.setAreaObj(areaObj);
            /*处理用户照片上传*/
    ...
    public String QueryUserInfo() {
        ...
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name, areaObj, name, birthDate, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        userInfoDAO.CalculateTotalPageAndRecordNumber(user_name, areaObj, name, birthDate, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryUserInfoOutputToExcel() { 
        ...
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name,areaObj,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "UserInfo信息记录"; 
        String[] headers = { "用户名","所在地区","姓名","性别","出生日期","用户照片","联系电话","邮箱","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getAreaObj().getAreaName(),
userInfo.getName(),userInfo.getGender(),new SimpleDateFormat("yyyy-MM-dd").format(userInfo.getBirthDate()),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getRegTime()});
        }
        /*
